-- =============================================================================
-- Carinho CRM - Schema SQL consolidado
-- =============================================================================
-- Consolida todas as estruturas das migrations para a aplicação funcionar
-- a partir de um único script (instalação limpa).
--
-- Migrations correspondentes:
--   2024_01_01_000001_create_domain_tables
--   2024_01_01_000002_create_users_table
--   2024_01_01_000003_create_leads_table
--   2024_01_01_000004_create_clients_table
--   2024_01_01_000005_create_care_needs_table
--   2024_01_01_000006_create_pipeline_stages_table
--   2024_01_01_000007_create_deals_table
--   2024_01_01_000008_create_proposals_table
--   2024_01_01_000009_create_contracts_table
--   2024_01_01_000010_create_consents_table
--   2024_01_01_000011_create_tasks_table
--   2024_01_01_000012_create_interactions_table
--   2024_01_01_000013_create_loss_reasons_table
--   2024_01_01_000014_create_job_tables
--   2024_01_01_000015_create_personal_access_tokens_table
--   2026_01_23_000001_add_traditional_practices_improvements
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- Domain tables (2024_01_01_000001_create_domain_tables)
-- -----------------------------------------------------------------------------
...
-- Domínios da migration 2026 (devem existir antes de clients)
...
-- Dados dos domínios (2024_01_01_000001)
...
INSERT INTO domain_patient_type (id, code, label) VALUES
  ...
  (2, 'pcd', 'PCD'),
  (3, 'tea', 'TEA'),
  (4, 'pos_operatorio', 'Pós-operatório');
...
-- Dados dos domínios (2026_01_23_000001)
...
-- -----------------------------------------------------------------------------
-- Users, auth, sessions (2024_01_01_000002_create_users_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Leads (2024_01_01_000003_create_leads_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Clients (2024_01_01_000004 + 2026_01_23 - campos adicionais)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Care needs (2024_01_01_000005_create_care_needs_table)
-- -----------------------------------------------------------------------------
...
CREATE TABLE care_needs (
  ...
  CONSTRAINT fk_care_needs_patient
    FOREIGN KEY (patient_type_id) REFERENCES domain_patient_type(id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- -----------------------------------------------------------------------------
-- Pipeline stages (2024_01_01_000006_create_pipeline_stages_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Deals (2024_01_01_000007 + 2026 - probability, weighted_value, next_action, etc.)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Proposals (2024_01_01_000008_create_proposals_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Contracts (2024_01_01_000009 + 2026 - renewal_alert_days, auto_renewal, etc.)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Consents (2024_01_01_000010_create_consents_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Tasks (2024_01_01_000011_create_tasks_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Interactions (2024_01_01_000012_create_interactions_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Loss reasons (2024_01_01_000013_create_loss_reasons_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Job tables (2024_01_01_000014_create_job_tables)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Personal access tokens (2024_01_01_000015_create_personal_access_tokens_table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Client events, reviews, referrals (2026_01_23_000001)
-- -----------------------------------------------------------------------------
...
CREATE TABLE client_events (
  ...
  CONSTRAINT fk_client_events_type
    FOREIGN KEY (event_type_id) REFERENCES domain_event_type(id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- -----------------------------------------------------------------------------
-- Indexes (sessions, jobs, tokens, leads, clients, deals, etc.)
-- -----------------------------------------------------------------------------
...
CREATE INDEX idx_clients_classification ON clients (classification_id);
CREATE INDEX idx_clients_next_review ON clients (next_review_date);

CREATE INDEX idx_care_needs_patient_type ON care_needs (patient_type_id);

CREATE INDEX idx_pipeline_order ON pipeline_stages (stage_order, active);
...
CREATE INDEX idx_contracts_client_status ON contracts (client_id, status_id);
CREATE INDEX idx_contracts_expiring ON contracts (end_date, status_id);
...
CREATE INDEX idx_tasks_lead_id ON tasks (lead_id, status_id);
...
CREATE INDEX idx_interactions_channel_time ON interactions (channel_id, occurred_at);

CREATE INDEX idx_loss_reasons_reason ON loss_reasons (reason);

CREATE INDEX idx_client_events_timeline ON client_events (client_id, occurred_at);
CREATE INDEX idx_client_events_type ON client_events (event_type_id);
CREATE INDEX idx_client_events_related ON client_events (related_id, related_type);

CREATE INDEX idx_client_reviews_date ON client_reviews (client_id, review_date);

CREATE INDEX idx_referrals_status ON client_referrals (referrer_client_id, status);
